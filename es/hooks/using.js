...
export function getScopeHookNames(scopeName) {
...
export function getScopeHooks(scopeName) {
  var scopeHooks = ScopesHooks[scopeName];
  return scopeHooks.map(function (_ref2) {
    var hookName = _ref2.hookName,
        opts = _ref2.opts;
    var hookEntity = getHookEntity(hookName);
...
function setHooksOpts(hooksConfig, scope) {
  var scopeName = getScopeName(scope); // 检测在当前scope上是否启用扩展
...
export function usingHooks(hookConfigs, scopes) {
  // 禁用所有
  ...
  registerHooks(needRegisterHooks); // 启用hook
...
export function configHooks(hookConfigs, scopes) {
  var configs = hookConfigs.map(function (hookConfig) {
    if (typeof hookConfig === 'string') {
      return {
        hookName: hookConfig,
